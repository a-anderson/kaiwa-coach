...
### Definition of Done
- [ ] Tab bar renders; clicking each tab updates the active tab without errors
- [ ] Gear icon in top bar opens/closes a placeholder settings panel overlay
- [ ] Esc key closes the settings overlay
- [ ] Narration tab hides the sidebar and expands the main panel to full width
- [ ] Switching tabs does not break the existing Chat flow (conversation list, chat, audio still work)
...
- Per-language level dropdowns:
  ...
  - All other languages: one CEFR dropdown — A1, A2, B1, B2, C1, C2 (A1 = beginner, displayed first)
...
### Definition of Done
- [ ] `poetry run pytest -q tests/test_storage_schema.py` — user_profile table, all columns, and default row asserted
...
- [ ] `poetry run pytest -q tests/test_api_settings.py` (new file) — GET returns defaults on fresh DB; POST updates `ja` and `ja_kanji` independently; subsequent GET reflects both; POST with invalid level `{"ja": "fluent"}` returns 422; POST with `{"ja_kanji": "Native"}` succeeds; POST with unknown language key returns 422
- [ ] `poetry run pytest -q tests/test_prompt_schemas.py tests/test_prompt_rendering_suite.py tests/test_prompt_loader.py` — all pass after prompt variable additions; rendering with empty string for `user_name` does not raise KeyError
- [ ] Manual: set name to "Ashley" and level to N3 for Japanese; send a turn; verify the reply is pitched at N3 level
- [ ] Manual: change level to N1 mid-conversation; next reply is noticeably more advanced without restarting
- [ ] No TypeScript errors in SettingsPanel; `npm run build` succeeds
...
| Order | Branch | Feature | Dependencies before merge |
|-------|--------|---------|--------------------------|
